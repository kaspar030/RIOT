...
{
    mbox_sync_t *mbox = arg;

    while (1) {
        msg_t test;
        mbox_sync_recv(mbox, &test);
    }
...
int main(void)
{
    ...
    mbox_sync_t mbox = { .senders.next = NULL, .waiter = NULL };

    thread_create(_stack, sizeof(_stack), (THREAD_PRIORITY_MAIN - 1),
                  THREAD_CREATE_STACKTEST, _second_thread, &mbox,
                  "second_thread");
