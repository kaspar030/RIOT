...
{
    (void)arg;
    svc_provider_t provider = { .owner = thread_get_active() };

    do {
        svc_request_t *req;
        while (!(req = svc_request_get(&_svc))) {
            svc_provide(&_svc, &provider);
            thread_event_get_this((thread_event_t *)&provider.list_entry);
        }
        // handle
        svc_reply(req, NULL, 0);
    } while (1);
...
int main(void)
{
    ...
    thread_create(_stack, sizeof(_stack), (THREAD_PRIORITY_MAIN -1),
                  THREAD_CREATE_STACKTEST, _second_thread, NULL,
                  "second_thread");
