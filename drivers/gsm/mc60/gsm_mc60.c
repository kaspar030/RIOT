                         ...
                         uint8_t *resultbuf, size_t result_len)
{
    ...
    for (unsigned i = 0; i < ncmds; i++) {
        res = at_send_cmd_wait_ok(at_dev, cmds[i], GSM_SERIAL_TIMEOUT);
        if (res) {
            goto out;
        }
    }
    ...
    if (strncmp(url, "https://", 8) == 0) {
        ...
        ncmds = sizeof(ssl_cmds)/sizeof(char*);
...
static int mc60_get_loc(gsm_t *gsmdev, char *lon, char *lat)
{
    ...
    int res = at_send_cmd_get_resp(&gsmdev->at_dev, "AT+QCELLLOC=1", buf, sizeof(buf), GSM_SERIAL_TIMEOUT * 90);
    mutex_unlock(&gsmdev->mutex);
...
static int _mc60_time_sync(gsm_t *gsmdev)
{
    LOG_INFO("gsm: mc60: synchronizing time...\n");
    mutex_lock(&gsmdev->mutex);

    int res = at_send_cmd_wait_ok(&gsmdev->at_dev, "AT+QNITZ", GSM_SERIAL_TIMEOUT * 120);
    if (res < 0) {
        res = -1;
    }
    res = at_send_cmd_wait_ok(&gsmdev->at_dev, "AT+CTZU", GSM_SERIAL_TIMEOUT * 120);
    if (res < 0) {
        res = -1;
    }
