 ...
 * time (last_wakeup + interval).
 *
 * When the function returns, @c last_wakeup is set to xtimer_now().
 *
 * This function can be used to create periodic wakeups.
 * @c last_wakeup should be set to xtimer_now() before first call of the function.
 *
 * If the result of (@c last_wakeup + usecs) would be in the past, the function sets
 ...
 *
 * This function sets a timer that will send a message @p offset microseconds from now.
 *
 ...
 * @return       < 0 on error, other value otherwise
 */
int xtimer_msg_receive_timeout64(msg_t *m, uint64_t us);
