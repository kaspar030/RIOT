...
ssize_t _manifest_post_handler(coap_pkt_t* pkt, uint8_t *buf, size_t len)
{
    ...
    if (riotboot_suit_putbytes(pkt->payload, pkt->payload_len, block1.offset, more) < 0) {
...
ssize_t _uuid_handler(coap_pkt_t* pkt, uint8_t *buf, size_t len, void *context)
{
    ...
    *pkt_pos++ = 0xFF;
    ...
    switch(type) {
        case 'v':
            uuid = suit_cbor_get_uuid_vendor();
            break;
        case 'c':
            uuid = suit_cbor_get_uuid_class();
            break;
        case 'd':
            uuid = suit_cbor_get_uuid_device();
            break;
        default:
            return -1;
    }
...
static ssize_t _time_handler(coap_pkt_t *pkt, uint8_t *buf, size_t len, void *context)
{
    (void)context;
    uint8_t time[16];
    size_t time_len = fmt_u64_dec((char*)time, riotboot_suit_get_time());
    return coap_reply_simple(pkt, COAP_CODE_205, buf, len,
