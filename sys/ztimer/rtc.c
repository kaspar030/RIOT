...
static uint32_t _gmt_civil_to_timestamp(unsigned y, unsigned m, unsigned d, unsigned h, unsigned mi, unsigned s)
{
    if (m <= 2) {
        y -= 1;
        m += 12;
    }
    return (365*y + y/4 - y/100 + y/400 + 3*(m+1)/5 + 30*m + d - 719561) *
...
void _timestamp_to_gmt_civil(struct tm *_tm, uint32_t epoch)
{
    ...
    if (e < 14) {
        struct tm tmp = { .tm_year=c-4716-1900, .tm_mon=e-1, .tm_mday=f, .tm_hour=h, .tm_min=m, .tm_sec=s };
        *_tm = tmp;
    }
    else {
        struct tm tmp = { .tm_year=c-4715-1900, .tm_mon=e-13, .tm_mday=f, .tm_hour=h, .tm_min=m, .tm_sec=s };
        *_tm = tmp;
    }
}
...
static uint32_t _ztimer_rtc_now(ztimer_dev_t *ztimer)
{
    ...
    return _gmt_civil_to_timestamp(time.tm_year + 1900, time.tm_mon, time.tm_mday, time.tm_hour, time.tm_min, time.tm_sec);
}
