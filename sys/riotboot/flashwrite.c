...
int riotboot_flashwrite_init(riotboot_flashwrite_t *state, int target_slot,
                             size_t offset)
{
                               ...
                               riotboot_hdr_t *hdr, size_t len)
{
    ...
    size_t hdr_space = (void *)hdr->start_addr - slot_start;

    void *firstpage;

    if (len < FLASHPAGE_SIZE) {
        firstpage = state->flashpage_buf;
        memset(firstpage, 0, hdr_space);
        memcpy(firstpage, hdr, len);
        memcpy(firstpage + hdr_space, \
               slot_start + hdr_space, \
               FLASHPAGE_SIZE - hdr_space);
    }
    else {
        firstpage = hdr;
    }
