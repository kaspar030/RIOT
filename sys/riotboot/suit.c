...
#define RIOTBOOT_SUIT_TIME_INTERVAL 300
#define RIOTBOOT_SUIT_TIME_SERVER   "fe80::12e2:d5ff:ff00:1d9"
#define RIOTBOOT_SUIT_TIME_URL      "/t"
...
/* Updater state */
static riotboot_suit_t _fw_man;

static ssize_t fetch_block(riotboot_suit_t *state, unsigned num, sock_udp_ep_t *local);
static bool _check_seq_no(riotboot_suit_t *state);
static bool _check_digest(riotboot_suit_t *state, uint8_t *digest);
static void *_riotboot_suit_thread(void *arg);
...
static ssize_t fetch_time_block(unsigned num, sock_udp_ep_t *local)
{
    ...
    ipv6_addr_from_str((ipv6_addr_t *)&remote.addr.ipv6, RIOTBOOT_SUIT_TIME_SERVER);
    ...
    pktpos += coap_opt_put_uri_path(pktpos, 0, RIOTBOOT_SUIT_TIME_URL);
    pktpos += coap_put_option_block2_req(pktpos, COAP_OPT_URI_PATH, num, 2);
    pkt.payload = pktpos;
    pkt.payload_len = 0;
    ...
    if (res > 0) {
        ...
        if (block2.offset + pkt.payload_len > sizeof(time_buf)) {
            LOG_ERROR("ota: received riotboot larger than expected: %u, aborting\n", block2.offset + pkt.payload_len);
            return -1;
        }
...
static void _update_time(void)
{
    ...
    xtimer_set_msg64(&timer, RIOTBOOT_SUIT_TIME_INTERVAL * US_PER_SEC, &time_msg, sched_active_pid);
    _fetch_time();
}
...
static bool _check_seq_no(riotboot_suit_t *state)
{
    const riotboot_hdr_t *cur = riotboot_slot_get_hdr(riotboot_slot_current());

    uint32_t seq_no;
    if (suit_cbor_get_seq_no(&state->manifest, &seq_no) != SUIT_OK) {
        LOG_WARNING("riotboot_suit: error getting sequence number\n");
        return false;
    }

    return cur->version <= seq_no;
}
...
static bool _check_digest(riotboot_suit_t *state, uint8_t *digest)
{
    ...
    if (!suit_cbor_payload_get_digest(&state->manifest, SUIT_DIGEST_TYPE_RAW, exp, &len)) {
...
static int _manifest_verify(riotboot_suit_t *state)
{
    cose_signature_t signature;
    ...
    if (suit_validate_manifest(&state->manifest, riotboot_suit_get_time()) != SUIT_OK) {
        LOG_WARNING("Conditionals failure, ignoring update\n");
        return -1;
    }
    if (!_check_seq_no(state)) {
        LOG_WARNING("Timestamp failure, ignoring update\n");
        return -1;
    }
    char path[128];
    char hostport[SOCK_HOSTPORT_MAXLEN];
    if (suit_cbor_get_url(&state->manifest, path, sizeof(path)) < 0) {
...
static int riotboot_suit_finish_update(riotboot_suit_t *state)
{
    uint8_t hdr_buf[sizeof(riotboot_hdr_t)];
    uint8_t digest[SHA256_DIGEST_LENGTH];

    riotboot_hdr_t *hdr = (riotboot_hdr_t *)hdr_buf;

    uint32_t payload_size;
    if (suit_cbor_payload_get_size(&state->manifest, &payload_size) != SUIT_OK) {
        LOG_WARNING("riotboot_suit: error getting payload size\n");
        return -1;
    }
    ...
    memcpy(&hdr->magic_number, "RIOT", 4);
    hdr->start_addr = ((uint32_t)riotboot_slot_get_hdr(riotboot_slot_other()) + sizeof(riotboot_hdr_t));
    sha256((uint32_t *)hdr->start_addr, payload_size, digest);
    if (!_check_digest(state, digest)) {
        LOG_WARNING("riotboot_suit: Digest check failure\n");
        return -1;
    }
    ...
    return riotboot_flashwrite_finish(&state->writer, hdr, sizeof(riotboot_hdr_t));
}
...
static ssize_t fetch_block(riotboot_suit_t *state, unsigned num, sock_udp_ep_t *local)
{
    ...
    uint32_t manifest_payload_size;
    if (suit_cbor_payload_get_size(&state->manifest, &manifest_payload_size) != SUIT_OK) {
        LOG_WARNING("riotboot_suit: error getting payload size\n");
        return -1;
    }
    ...
    if (res > 0) {
        ...
        if (block2.offset == 0) {
            riotboot_flashwrite_init(&_fw_man.writer, riotboot_slot_other(), sizeof(riotboot_hdr_t));
            _fw_man.writer.flashpage++;
        }
        if (block2.offset + pkt.payload_len > manifest_payload_size) {
            LOG_ERROR("ota: received riotboot larger than expected: %u, aborting\n", block2.offset + pkt.payload_len);
            return -1;
        }
        riotboot_flashwrite_putbytes(&_fw_man.writer, pkt.payload,
...
static int _get_update(void)
{
    ...
    if (riotboot_suit_finish_update(&_fw_man) < 0) {
        LOG_WARNING("ota: Error finalizing update\n");
    }
    else {
        LOG_INFO("ota: Succesfully updated riotboot\n");
    }
out:
    mutex_unlock(&_fw_man.lock);
    return res;
...
void riotboot_suit_run(void)
{
    ...
    thread_create(_stack, STACKSIZE, PRIO, THREAD_CREATE_STACKTEST,
                  _riotboot_suit_thread, NULL, TNAME);
}
